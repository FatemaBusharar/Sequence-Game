window.addEventListener('DOMContentLoaded', () => {
    ...
    function updateHints() {
        document.querySelectorAll('.board-card')
            .forEach(card => card.classList.remove('hint-highlight'))
        ...
        document.querySelectorAll('.board-card').forEach(boardCard => {
            ...
            if (boardCard.dataset.value !== selectedCard.dataset.value ||
                boardCard.dataset.suit !== selectedCard.dataset.suit) return
            let found = false
            for (let i = 0; i < ROWS; i++) {
                for (let j = 0; j < COLS; j++) {
                    if (grid[i][j] === boardCard) { 
                        boardCard.dataset.ROWS !== i
                        boardCard.dataset.COLS !== j
                        found = true
                        break
                     }
                     if (found) break
    ...
    drawBtn.addEventListener('click', () => {
        ...
        if (player.cards.length >= 7){
            showMessage('You already have 7 cards , you cannoot draw more')
            return
        }
        const freeBoard = document.querySelectorAll('.board-card:not(.taken):not(.free)')
